...
struct SimCliOptions {
  ...
  std::string controlScriptPath;
  bool resetSession = false;
  long delayCapMs = -1;
  std::string device = "x4";
  std::string heapProfile = "device";
  long heapFreeBytes = -1;
  long heapLargestBytes = -1;
};
...
void resetEmulatorSessionState() {
  ...
  const fs::path onlineCacheRoot = dataRoot / "online_cache";
  ...
  ec.clear();
  fs::remove_all(onlineCacheRoot, ec);
...
bool g_controlReadyForced = false;
...
std::string screenSnapshotJson() {
  ...
  out << ",\"heapFree\":" << SimHeap::freeHeap();
  out << ",\"heapMinFree\":" << SimHeap::minFreeHeap();
  out << ",\"heapLargest\":" << SimHeap::largestFreeBlock();
  out << ",\"heapTotal\":" << SimHeap::heapSize();
...
bool equalsAsciiCaseInsensitive(const std::string& left, const std::string& right) {
  return toUpperAscii(trimAscii(left)) == toUpperAscii(trimAscii(right));
}
...
bool visibleListContains(const ScreenDebugSnapshot& screen, const std::string& value, int& targetVisibleIndex) {
  ...
  for (size_t i = 0; i < screen.list.visibleItems.size(); ++i) {
    const auto& item = screen.list.visibleItems[i];
    if (equalsAsciiCaseInsensitive(item.title, value) || equalsAsciiCaseInsensitive(item.subtitle, value) ||
        equalsAsciiCaseInsensitive(item.value, value)) {
...
bool visibleMenuContains(const ScreenDebugSnapshot& screen, const std::string& value, int& targetIndex) {
  targetIndex = -1;
  for (size_t i = 0; i < screen.buttonMenu.labels.size(); ++i) {
    if (equalsAsciiCaseInsensitive(screen.buttonMenu.labels[i], value)) {
...
bool controlChannelReady() {
  ...
  const auto screen = SCREEN_DEBUG.getSnapshot();
  const bool hasVisibleActivity = !activityManager.getCurrentActivityName().empty() || !screen.activityName.empty();
  const bool hasUiSurface = !screen.buttonMenu.labels.empty() || screen.list.itemCount > 0 || !screen.headerTitle.empty() ||
                            !screen.bodyPrimaryText.empty();
  return hasVisibleActivity && hasUiSurface;
}

void maybeEmitControlReady() {
  if (!g_cliOptions.controlStdio || g_controlReadyEmitted) return;
  if (!controlChannelReady()) {
    if (millis() < 3000) {
      return;
    }
    g_controlReadyForced = true;
  }
  g_controlReadyEmitted = true;
  const char* eventName = g_controlReadyForced ? "ready_forced" : "ready";
  emitControlJson(std::string("{\"ok\":true,\"event\":\"") + eventName + "\",\"state\":" + buildCompactStateJson() + "}");
}
...
void printHelp() {
  ...
  std::puts("  --device <x3|x4>");
  std::puts("  --heap-profile <device|x3|x4|desktop>");
  std::puts("  --heap-free-bytes <bytes>");
  std::puts("  --heap-largest-bytes <bytes>");
  std::puts("  --control-stdio");
  std::puts("  --control-script <path>");
  std::puts("  --help");
}
...
bool parseArgs(int argc, char** argv, SimCliOptions& options) {
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help" || arg == "-h") {
      ...
      const char* value = requireValue("--heap-profile");
      if (!value) return false;
      options.heapProfile = value;
    } else if (arg == "--heap-free-bytes") {
      const char* value = requireValue("--heap-free-bytes");
      if (!value || !parseLongArg(value, options.heapFreeBytes)) return false;
    } else if (arg == "--heap-largest-bytes") {
      const char* value = requireValue("--heap-largest-bytes");
      if (!value || !parseLongArg(value, options.heapLargestBytes)) return false;
    } else if (arg == "--control-stdio") {
      options.controlStdio = true;
    } else if (arg == "--control-script") {
      const char* value = requireValue("--control-script");
      if (!value) return false;
      options.controlScriptPath = value;
    } else {
...
void applyCliOptions(const SimCliOptions& options) {
  ...
  if (!options.device.empty()) setEnvVar("SIM_DEVICE", options.device);
  if (!options.heapProfile.empty()) setEnvVar("SIM_HEAP_PROFILE", options.heapProfile);
  if (options.heapFreeBytes >= 0) setEnvVar("SIM_HEAP_FREE", std::to_string(options.heapFreeBytes));
  if (options.heapLargestBytes >= 0) setEnvVar("SIM_HEAP_LARGEST", std::to_string(options.heapLargestBytes));
}
...
void startControlWait(PendingControlWait::Kind kind, const std::string& value, unsigned long timeoutMs) {
  PendingControlWait wait{kind, value, millis(), timeoutMs, 0};
  std::string matchedText;
  if (screenMatchesWait(wait, matchedText)) {
    emitControlJson(std::string("{\"ok\":true,\"event\":\"wait_satisfied\",\"waitType\":\"") + controlWaitKindName(kind) +
                    "\",\"value\":\"" + jsonEscape(value) + "\",\"matched\":\"" + jsonEscape(matchedText) +
                    "\",\"state\":" + buildCompactStateJson() + "}");
    g_pendingControlWait.reset();
    return;
  }

  g_pendingControlWait = wait;
  emitControlJson(std::string("{\"ok\":true,\"event\":\"wait_started\",\"waitType\":\"") + controlWaitKindName(kind) +
                  "\",\"value\":\"" + jsonEscape(value) + "\",\"timeoutMs\":" + std::to_string(timeoutMs) + "}");
}

std::optional<ControlCommand> parseControlCommandLine(const std::string& line) {
  std::string sanitizedLine = line;
  if (sanitizedLine.rfind("\xEF\xBB\xBF", 0) == 0) {
    sanitizedLine.erase(0, 3);
  }

  std::istringstream iss(sanitizedLine);
  std::string name;
...
bool loadControlScript(const std::string& path) {
  std::ifstream input(path);
  if (!input) {
    std::fprintf(stderr, "[SIM] Failed to open control script: %s\n", path.c_str());
    return false;
  }
  ...
  while (true) {
    ...
    switch (cmd.type) {
      ...
      case ControlCommand::Type::TypeText:
        ...
        break;
      case ControlCommand::Type::WaitMs:
        startControlWait(PendingControlWait::Kind::Delay, std::to_string(cmd.timeoutMs) + "ms", cmd.timeoutMs);
        return;
      ...
      case ControlCommand::Type::Screenshot:
        ...
        break;
      case ControlCommand::Type::WaitActivity:
        startControlWait(PendingControlWait::Kind::Activity, cmd.arg, cmd.timeoutMs);
        return;
      case ControlCommand::Type::WaitHeader:
        startControlWait(PendingControlWait::Kind::Header, cmd.arg, cmd.timeoutMs);
        return;
      case ControlCommand::Type::WaitBody:
        startControlWait(PendingControlWait::Kind::Body, cmd.arg, cmd.timeoutMs);
        return;
      case ControlCommand::Type::WaitListItem:
        startControlWait(PendingControlWait::Kind::ListItem, cmd.arg, cmd.timeoutMs);
        return;
      case ControlCommand::Type::WaitMenuItem:
        startControlWait(PendingControlWait::Kind::MenuItem, cmd.arg, cmd.timeoutMs);
        return;
      case ControlCommand::Type::WaitSelectedListItem:
        startControlWait(PendingControlWait::Kind::SelectedListItem, cmd.arg, cmd.timeoutMs);
        return;
      case ControlCommand::Type::WaitSelectedMenuItem:
        startControlWait(PendingControlWait::Kind::SelectedMenuItem, cmd.arg, cmd.timeoutMs);
        return;
      case ControlCommand::Type::WaitPopup:
        startControlWait(PendingControlWait::Kind::Popup, cmd.arg, cmd.timeoutMs);
        return;
...
bool hasSameScreenActionCompleted(const PendingControlAction& action, const ScreenDebugSnapshot& screen) {
  ...
  return screen.frameId > action.sourceFrameId && (menuStateChanged || listStateChanged || bodyChanged || popupChanged);
}
...
void updateControlAction() {
  ...
  if (action.kind == PendingControlAction::Kind::ActivateVisibleListItem) {
    if (action.confirmQueued) {
      const bool transitioned =
          screen.activityName != action.sourceActivityName || screen.headerTitle != action.sourceHeaderTitle;
      if (transitioned && activityManager.getPendingActivityName().empty() &&
...
int main(int argc, char** argv) {
  ...
  SimHeap::initializeFromEnvironment();
  std::printf("[SIM] Heap profile=%s device=%s %s\n", SimHeap::profileName(SimHeap::currentProfile()),
              g_cliOptions.device.c_str(), SimHeap::summary().c_str());
  ...
  if (!g_cliOptions.controlScriptPath.empty()) {
    loadControlScript(g_cliOptions.controlScriptPath);
  }
