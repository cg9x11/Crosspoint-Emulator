...
function Get-HeapMetricsFromState {
  param($State)
  ...
  $hasHeapFields = @("heapFree", "heapMinFree", "heapLargest", "heapTotal") | Where-Object {
    $diagnostics.PSObject.Properties.Name -contains $_
  }
  if ($hasHeapFields.Count -lt 4) {
    return $null
  }

  $heapTotal = [double]$diagnostics.heapTotal
  $heapFree = [double]$diagnostics.heapFree
  $heapMinFree = [double]$diagnostics.heapMinFree
  $heapLargest = [double]$diagnostics.heapLargest
  $used = $heapTotal - $heapFree
  $peakUsed = $heapTotal - $heapMinFree

  [pscustomobject]@{
    HeapFree = [int]$heapFree
    HeapMinFree = [int]$heapMinFree
    HeapLargest = [int]$heapLargest
    HeapTotal = [int]$heapTotal
    UsedBytes = [int]$used
    PeakUsedBytes = [int]$peakUsed
    UsedPercent = if ($heapTotal -gt 0) { [math]::Round(($used / $heapTotal) * 100.0, 1) } else { 0.0 }
    PeakUsedPercent = if ($heapTotal -gt 0) { [math]::Round(($peakUsed / $heapTotal) * 100.0, 1) } else { 0.0 }
  }
}
...
function Invoke-BenchmarkRun {
  ...
  $commandFile = Join-Path ([System.IO.Path]::GetTempPath()) ("crosspoint-bench-" + [guid]::NewGuid().ToString("N") + ".txt")
  $stdoutFile = Join-Path ([System.IO.Path]::GetTempPath()) ("crosspoint-bench-" + [guid]::NewGuid().ToString("N") + ".stdout.log")
  $stderrFile = Join-Path ([System.IO.Path]::GetTempPath()) ("crosspoint-bench-" + [guid]::NewGuid().ToString("N") + ".stderr.log")
  Write-Utf8NoBomFile -Path $commandFile -Content ($Commands -join [Environment]::NewLine)
  ...
  try {
    ...
    foreach ($path in @($commandFile, $stdoutFile, $stderrFile)) {
      if (Test-Path $path) {
...
$benchmarks = @(
  ...
  @{
    ...
    Commands = @(
      ...
      "ACTIVATE_VISIBLE_LIST_ITEM Sources",
      "WAIT_HEADER Sources 15000",
      "ACTIVATE_VISIBLE_LIST_ITEM Hako Server LAN",
      "WAIT_HEADER Hako Server LAN 30000",
      "TAP LEFT 180",
    ...
    End = { param($e) $e.event -eq "wait_satisfied" -and $e.waitType -eq "list_item" -and $e.value -eq "Sword of the Demon Hunter" }
  },
  @{
    ...
    Commands = @(
      ...
      "WAIT_HEADER Sources 15000",
      "ACTIVATE_VISIBLE_LIST_ITEM Hako Server LAN",
      "WAIT_HEADER Hako Server LAN 15000",
      "QUIT"
    )
    Start = { param($e) $e.event -eq "action_completed" -and $e.action -eq "activate_visible_list_item" -and $e.value -eq "Sources" }
    End = { param($e) $e.event -eq "wait_satisfied" -and $e.waitType -eq "header" -and $e.value -eq "Hako Server LAN" }
    Validate = {
      param($Events)
      $endEvent = Get-MatchingEvent -Events $Events -Predicate { param($e) $e.event -eq "wait_satisfied" -and $e.waitType -eq "header" -and $e.value -eq "Hako Server LAN" }
  ...
  @{
    ...
    Commands = @(
      ...
      "ACTIVATE_VISIBLE_LIST_ITEM Hako Server LAN",
      "WAIT_HEADER Hako Server LAN 15000",
      "WAIT_BODY Summary 30000",
      "QUIT"
    )
    Start = { param($e) $e.event -eq "wait_satisfied" -and $e.waitType -eq "header" -and $e.value -eq "Hako Server LAN" }
  ...
  @{
    ...
    Commands = @(
      ...
      "ACTIVATE_VISIBLE_LIST_ITEM Sources",
      "WAIT_HEADER Sources 15000",
      "ACTIVATE_VISIBLE_LIST_ITEM Truyen Full Server LAN",
      "WAIT_HEADER Truyen Full Server LAN 30000",
      "TAP LEFT 180",
      "WAIT_ACTIVITY KeyboardEntry 10000",
      "TYPE_TEXT tien",
  ...
  @{
    ...
    Commands = @(
      ...
      "WAIT_HEADER Sources 15000",
      "ACTIVATE_VISIBLE_LIST_ITEM Truyen Full Server LAN",
      "WAIT_HEADER Truyen Full Server LAN 15000",
      "QUIT"
    )
    Start = { param($e) $e.event -eq "action_completed" -and $e.action -eq "activate_visible_list_item" -and $e.value -eq "Sources" }
    End = { param($e) $e.event -eq "wait_satisfied" -and $e.waitType -eq "header" -and $e.value -eq "Truyen Full Server LAN" }
    Validate = {
      param($Events)
      $endEvent = Get-MatchingEvent -Events $Events -Predicate { param($e) $e.event -eq "wait_satisfied" -and $e.waitType -eq "header" -and $e.value -eq "Truyen Full Server LAN" }
  ...
  @{
    ...
    Start = { param($e) $e.event -eq "wait_satisfied" -and $e.waitType -eq "header" -and $e.value -eq "Truyen Full Server LAN" }
    End = { param($e) $e.event -eq "wait_satisfied" -and $e.waitType -eq "body" -and $e.value -eq "Summary" }
    Validate = {
      param($Events)
      $endEvent = Get-MatchingEvent -Events $Events -Predicate { param($e) $e.event -eq "wait_satisfied" -and $e.waitType -eq "body" -and $e.value -eq "Summary" }
...
try {
  ...
  foreach ($benchmark in $benchmarks) {
    ...
    $startEvent = Get-MatchingEvent -Events $events -Predicate $benchmark.Start
    $endEvent = Get-MatchingEvent -Events $events -Predicate $benchmark.End
    $startHeap = if ($null -ne $startEvent) { Get-HeapMetricsFromState -State $startEvent.state } else { $null }
    $endHeap = if ($null -ne $endEvent) { Get-HeapMetricsFromState -State $endEvent.state } else { $null }
    $results.Add([pscustomobject]@{
        Name = $benchmark.Name
        StartMs = $startMs
        EndMs = $endMs
        DurationMs = ($endMs - $startMs)
        HeapTotal = if ($null -ne $endHeap) { $endHeap.HeapTotal } else { $null }
        HeapFree = if ($null -ne $endHeap) { $endHeap.HeapFree } else { $null }
        HeapMinFree = if ($null -ne $endHeap) { $endHeap.HeapMinFree } else { $null }
        HeapLargest = if ($null -ne $endHeap) { $endHeap.HeapLargest } else { $null }
        UsedPercent = if ($null -ne $endHeap) { $endHeap.UsedPercent } else { $null }
        PeakUsedPercent = if ($null -ne $endHeap) { $endHeap.PeakUsedPercent } else { $null }
        StartHeapFree = if ($null -ne $startHeap) { $startHeap.HeapFree } else { $null }
        StartHeapMinFree = if ($null -ne $startHeap) { $startHeap.HeapMinFree } else { $null }
      })
  }

  $results | Sort-Object DurationMs -Descending | Format-Table Name,DurationMs,HeapTotal,HeapFree,HeapMinFree,HeapLargest,UsedPercent,PeakUsedPercent -AutoSize
} finally {
