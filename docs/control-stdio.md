...
- preview cover-failure fallback on `Truyen Full`
...
An end-to-end remote smoke test is available at:

```powershell
.\tools\remote-smoke.ps1
```

This boots the emulator on the `x3` heap profile, starts File Transfer in STA mode, verifies:

- `GET /remote`
- `GET /api/remote/status`
- `GET /api/remote/screen.bmp`
- `POST /api/remote/button`
- `POST /api/remote/text`

It also checks the full round-trip back into `KOReader Sync` so the submitted remote text is visible on-device again.
